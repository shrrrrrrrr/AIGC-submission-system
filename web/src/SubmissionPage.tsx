...
export function SubmissionPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (id) {
          ...
          if (data.submission.currentStatus !== "draft") throw new Error("该投稿版本只读，请返回工作台");
  ...
  const status = busy ? "正在保存…" : dirty ? "有未保存的修改" : time ? `已保存于 ${time} · 未正式提交` : "尚未创建草稿";
    ...
    {loading ? <p role="status">正在读取账户与草稿…</p> : unauthenticated ? <div className="submission-card"><h2>登录后开始投稿</h2><p>草稿保存在你的账号下，可以在下次登录后继续填写。</p><a className="button button-cinnabar" href="#login">登录账号 ↗</a></div> : <div className="submission-layout">
      ...
      <form className="submission-card" onSubmit={save}>
        ...
        <fieldset disabled={busy} className="draft-fields">
          ...
          {step === 5 && <><div className="review-list"><div className="review-row"><span>作品名称</span><span>{draft.title || "待填写"}</span></div><div className="review-row"><span>投稿方向</span><span>{directions[draft.direction]}</span></div>{(["mainWork", "makingOf", "guideVideo"] as const).map((purpose) => <div className="review-row" key={purpose}><span>{{ mainWork: "主体作品", makingOf: "制作解析", guideVideo: "导览视频" }[purpose]}</span><span>{links[purpose] ? "等待预检" : "待填写 / 依作品形式要求"}</span></div>)}</div><p id="submit-unavailable" className="form-notice">链接预检尚未开放，当前只能保存草稿。</p><button className="button button-cinnabar" disabled aria-describedby="submit-unavailable" type="button">确认提交</button></>}
        </fieldset>
        <div className="submission-actions"><button className="button button-cinnabar" type="submit" disabled={busy}>{busy ? "正在保存…" : "保存草稿"}</button>{step < 5 && <button className="button button-outline" type="button" disabled={busy} onClick={() => setStep(step + 1)}>下一步：{steps[step + 1]} →</button>}</div>
      </form>
    </div>}
  </section>;
}
